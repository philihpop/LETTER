...
class RQVAE(nn.Module):
    ...
    def compute_loss(self, out, quant_loss, emb_idx, dense_out, xs=None):
        ...
        # CF_Loss
        if self.alpha > 0:
            cf_embedding_in_batch = self.cf_embedding[emb_idx.cpu().numpy()]
            cf_embedding_in_batch = torch.from_numpy(cf_embedding_in_batch).to(xs.device).float()
            cf_loss = self.CF_loss(dense_out, cf_embedding_in_batch)
        else:
            cf_loss = torch.tensor(0.0, device=xs.device)
